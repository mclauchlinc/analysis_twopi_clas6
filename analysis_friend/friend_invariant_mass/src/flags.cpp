...
Flags::Flags(){}
	...
	void Flags::ID_Flag(std::string argi_){
		...
		if(str_len > 9){
			...
			if(str.substr(0,9) == _sim_file_pim_){
				_sim_pim_loc = str.substr(9,str_len);
				std::cout<<"\tIDed: Sim PIM File is " <<_sim_pim_loc <<"\n";
			}
			if(str.substr(0,9) == _sim_file_pro_){
				_sim_pro_loc = str.substr(9,str_len);
				std::cout<<"\tIDed: Sim pro File is " <<_sim_pro_loc <<"\n";
			}
			if(str.substr(0,9) == _sim_file_pip_){
				_sim_pip_loc = str.substr(9,str_len);
				std::cout<<"\tIDed: Sim pip File is " <<_sim_pip_loc <<"\n";
			}
			if(str.substr(0,9) == _exp_file_pim_){
				_exp_pim_loc = str.substr(9,str_len);
				std::cout<<"\tIDed: exp PIM File is " <<_exp_pim_loc <<"\n";
			}
			if(str.substr(0,9) == _exp_file_pro_){
				_exp_pro_loc = str.substr(9,str_len);
				std::cout<<"\tIDed: exp pro File is " <<_exp_pro_loc <<"\n";
			}
			if(str.substr(0,9) == _exp_file_pip_){
				_exp_pip_loc = str.substr(9,str_len);
				std::cout<<"\tIDed: exp pip File is " <<_exp_pip_loc <<"\n";
			}
		}
		if(str_len >= 11){
			if(str.substr(0,11) == _empty_file_pim_){
				_empty_pim_loc = str.substr(11,str_len);
				std::cout<<"\tIDed: empty PIM File is " <<_empty_pim_loc <<"\n";
			}
			if(str.substr(0,11) == _empty_file_pro_){
				_empty_pro_loc = str.substr(11,str_len);
				std::cout<<"\tIDed: empty pro File is " <<_empty_pro_loc <<"\n";
			}
			if(str.substr(0,11) == _empty_file_pip_){
				_empty_pip_loc = str.substr(11,str_len);
				std::cout<<"\tIDed: empty pip File is " <<_empty_pip_loc <<"\n";
			}
		}
		if(str_len >= 12){
			if(str.substr(0,12) == _sim_no_rad_file_pim_){
				_sim_no_rad_pim_loc = str.substr(12,str_len);
				std::cout<<"\tIDed: sim_no_rad PIM File is " <<_sim_no_rad_pim_loc <<"\n";
			}
			if(str.substr(0,12) == _sim_no_rad_file_pro_){
				_sim_no_rad_pro_loc = str.substr(12,str_len);
				std::cout<<"\tIDed: sim_no_rad pro File is " <<_sim_no_rad_pro_loc <<"\n";
			}
			if(str.substr(0,12) == _sim_no_rad_file_pip_){
				_sim_no_rad_pip_loc = str.substr(12,str_len);
				std::cout<<"\tIDed: sim_no_rad pip File is " <<_sim_no_rad_pip_loc <<"\n";
			}
		}
        if(str_len >= 13){
	...
	//File Names
	std::string Flags::Sim_File(std::string var_set_){
		if(var_set_ == _var_set_[0]){ return _sim_file_pim_;}
		else if(var_set_ == _var_set_[1]){ return _sim_file_pro_;}
		else if(var_set_ == _var_set_[2]){ return _sim_file_pip_;}
	}
	std::string Flags::Exp_File(std::string var_set_){
		if(var_set_ == _var_set_[0]){ return _exp_file_pim_;}
		else if(var_set_ == _var_set_[1]){ return _exp_file_pro_;}
		else if(var_set_ == _var_set_[2]){ return _exp_file_pip_;}
	}
	std::string Flags::Empty_File(std::string var_set_){
		if(var_set_ == _var_set_[0]){ return _empty_file_pim_;}
		else if(var_set_ == _var_set_[1]){ return _empty_file_pro_;}
		else if(var_set_ == _var_set_[2]){ return _empty_file_pip_;}
	}
	std::string Flags::No_Rad_File(std::string var_set_){
		if(var_set_ == _var_set_[0]){ return _sim_no_rad_file_pim_;}
		else if(var_set_ == _var_set_[1]){ return _sim_no_rad_file_pro_;}
		else if(var_set_ == _var_set_[2]){ return _sim_no_rad_file_pip_;}
	}
