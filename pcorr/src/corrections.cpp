...
float corr::p_corr_e(float p_e_, float theta_e_, float phi_e_, int run_, bool centered_, int sector_){
	...
	if(centered_){
		for(int i=0; i<4; i++){
			for(int j=0; j<3; j++){
				phi_const[i] += _p_e_par[run_][sector_][i][j]*pow(theta_e_,2-j);
			}
		}
	...
	for(int i=0; i<4; i++){
		for(int j=0; j<3; j++){
			phi_const[i] += _p_e_par[run_][sector][i][j]*pow(theta_e_,2-j);
		}
	}
...
float corr::theta_e_corr(float theta_e_, float phi_e_, int run_, bool centered_, int sector_){
	...
	if(centered_){
		for(int i=0; i<5; i++){
			for(int j=0; j<3; j++){
				phi_const[i] += _angle_e_par[run_][sector_][i][j]*pow(theta_e_,2-j);
			}
		}
